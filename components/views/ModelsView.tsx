...
const ModelsView: React.FC<ModelsViewProps> = ({ models, onAdd, onUpdate, onDelete }) => {
  ...
  const [deleteModal, setDeleteModal] = useState<{ isOpen: boolean; model: Model | null }>({
    isOpen: false,
    model: null
  });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-100 uppercase tracking-tight italic">Modelos RBC</h2>
          <p className="text-slate-400 text-sm">Gestão de ferramentas e ativos técnicos da equipe.</p>
        </div>
        <button
          onClick={() => openModal()}
      ...
      <div className="bg-slate-900 rounded-2xl shadow-sm border border-slate-800 overflow-hidden">
        ...
        <div className="overflow-x-auto">
            ...
            <thead>
                ...
                <th className="p-5 font-bold text-slate-500 text-[10px] uppercase tracking-widest">Tipo / Ativo</th>
            ...
            <tbody className="divide-y divide-slate-800/50">
                {filteredModels.length === 0 ? (
                    <tr>
                        <td colSpan={4} className="p-12 text-center text-slate-500 italic text-sm">Nenhum modelo registrado na base de dados.</td>
                    </tr>
                ) : (
                    filteredModels.map((m) => (
                    <tr key={m.id} className="hover:bg-slate-800/40 group transition-colors">
                        <td className="p-5">
                            <div className="flex items-center gap-4">
                                ...
                                <span className="font-bold text-slate-100 uppercase tracking-tight">{m.type}</span>
                            </div>
                        </td>
                        <td className="p-5">
                            <span className="text-sm text-slate-400 font-bold uppercase">{m.brand || '---'}</span>
                        </td>
                        <td className="p-5">
                            <span className="text-sm text-slate-400 font-bold uppercase tracking-tight">{m.model || '---'}</span>
                        </td>
                        ...
                        <div className="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all transform group-hover:translate-x-0 translate-x-2">
                            ...
                            <button 
                                type="button"
                                onClick={() => setDeleteModal({ isOpen: true, model: m })} 
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-slate-900 rounded-2xl shadow-2xl border border-slate-800 w-full max-w-md p-8 animate-in zoom-in-95 duration-200 overflow-y-auto max-h-[95vh]">
            <h3 className="text-xl font-black mb-8 text-white uppercase italic tracking-tight border-l-4 border-red-600 pl-4">{editingId ? 'Editar Modelo' : 'Novo Modelo'}</h3>
            <form onSubmit={handleSubmit} className="space-y-5">
              
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2 ml-1">Tipo de Ativo</label>
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2 ml-1">Modelo Comercial</label>
                  <div className="relative">
              ...
              <div className="flex flex-col gap-3 mt-10">
                <button type="submit" className="w-full py-4 bg-red-600 text-white rounded-xl hover:bg-red-700 font-black transition-all shadow-xl shadow-red-900/30 uppercase tracking-widest text-sm active:scale-95">Salvar Registro</button>
      ...
      <DeleteConfirmModal 
        ...
        itemName={`${deleteModal.model?.type} - ${deleteModal.model?.model}` || ''}
        title="Excluir Modelo"
        description="Esta ação removerá o modelo permanentemente do inventário da RBC Motorsport."
        onClose={() => setDeleteModal({ isOpen: false, model: null })}
        onConfirm={() => deleteModal.model && onDelete(deleteModal.model.id)}
      />
    </div>
  );
};
